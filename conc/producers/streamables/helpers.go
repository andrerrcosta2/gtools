...
// OpenCloseableSync is a std function that opens a domain.CloseableStreamable[T] synchronously
...
// OpenCloseableAsync is a std function that opens a domain.CloseableStreamable[T] asynchronously
// and fills it with the given values. It uses a goroutine to send all values
// to the channel and then closes the channel. If a signal is sent to the
// signal channel, it stops sending values and closes the channel.
...
// StreamCloseable is a std function that sends a value to a channel if the supplier isn't closed.
