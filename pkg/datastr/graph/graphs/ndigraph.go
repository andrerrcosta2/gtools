...
func UndirectOf[G gtools.SortableOf](nods ...G) *UndirectGraphOf[G] {
...
var _ graph.Graph[gtools.SortableOf] = (*UndirectGraphOf[gtools.SortableOf])(nil)
var _ graph.SingleEdgedGraph[gtools.SortableOf] = (*UndirectGraphOf[gtools.SortableOf])(nil)
var _ graph.GraphOf[gtools.SortableOf] = (*UndirectGraphOf[gtools.SortableOf])(nil)
...
func (g *UndirectGraphOf[G]) AddEdge(from, to G) {
	graph.addUndirectEdgeOfIfNodesExist[G](g.adj, from, to)
}
...
// *SingleTypedEdge[G].
func (g *UndirectGraphOf[G]) Edges() []*graph.SingleTypedEdge[G] {
	...
	for key, value, ok := iterator.Next(); ok; key, value, ok = iterator.Next() {
		...
		for _, neighbor := range value {
			...
			if g.comparator.Compare(key, neighbor) < 0 {
				...
				edges.Add(graph.NewEdge(neighbor, key))
			}
		}
	}
...
var _ graph.Graph[int] = (*UndirectGraph[int])(nil)
var _ graph.SingleEdgedGraph[int] = (*UndirectGraph[int])(nil)
var _ graph.OrderedGraph[int] = (*UndirectGraph[int])(nil)

// UndirectGraph is a graph with undirected edges.
type UndirectGraph[G constraints.Ordered] struct {
	adj map[G][]G
}
...
func (g *UndirectGraph[G]) AddNode(node G) {
	graph.addNodeIfNotExists(&g.adj, node)
}
...
//
// Finally, the function updates the adjacency list with the new neighbor lists.
func (g *UndirectGraph[G]) AddEdge(from, to G) {
	graph.addUndirectEdgeIfNodesExist(&g.adj, from, to)
}
...
// and returns a slice of edges, where each edge is represented as a
// *SingleTypedEdge[G].
func (g *UndirectGraph[G]) Edges() []*graph.SingleTypedEdge[G] {
	// Create a set to store unique edges
	edges := sets.SortableOf[*graph.SingleTypedEdge[G]]()
