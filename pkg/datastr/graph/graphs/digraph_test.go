...
import (
	...
	graph2 "github.com/andrerrcosta2/gtools/pkg/nums/graph"
...
func TestDirectedGraph_AddEdge(t *testing.T) {
	...
	var expEdges iterables.Slice[*graph2.SingleTypedEdge[testsortables.TestNode]]
	...
	testsortables.RandomTestNodes(30, "node").
		Operation(func(i int, n *iterables.Slice[testsortables.TestNode]) {
			...
			if i%3 == 0 && i+1 < n.Len() && i-1 >= 0 {
				...
				expEdges.Append(graph2.NewEdge(it, that))
			}
		})

	expEdges.Each(func(e *graph2.SingleTypedEdge[testsortables.TestNode]) {
...
func TestDirectedGraph_Edges(t *testing.T) {
	...
	arrays.ContainsAllBy(&edgesN, &expN, func(e1, e2 *graph2.SingleTypedEdge[testsortables.TestNode]) bool {
