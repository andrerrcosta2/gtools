...
func addNodeIfNotExists[G constraints.Ordered](adj *map[G][]G, node G) {
	if nodes, err := getNodes(adj, node); nodes.Len() == 0 && err == nil {
		(*adj)[node] = []G{}
	}
}

func addUndirectEdgeOfIfNodesExist[G gtools.SortableOf](adj maps.StructMap[G, []G], from, to G) {
...
func addUndirectEdgeIfNodesExist[G constraints.Ordered](adj *map[G][]G, from, to G) error {
...
func addDirectEdgeIfNodesExist[G constraints.Ordered](adj *map[G][]G, from, to G) error {
	nodes, err := getNodes(adj, from, to)
	if nodes.Len() != 2 {
		return fmt.Errorf("The nodes %v and/or %v do not exist in the graph: %v\n", from, to, nodes)
	}
...
func getNodes[G constraints.Ordered](adj *map[G][]G, nodes ...G) (*maps.EntrySet[G, []G], error) {
...
func IsConnectedOf[T gtools.SortableOf](g *graphs.UndirectGraphOf[T]) bool {
...
func IsDisconnectedOf[T gtools.SortableOf](g *graphs.UndirectGraphOf[T]) bool {
