...
func NewMergeSort[T prim.Ordered](comparator comparables.Comparator[T]) *MergeSort[T] {
	return &MergeSort[T]{
...
func (m *MergeSort[T]) SortP(arr *[]*T) {
	m.pmerge(arr)
}

func (m *MergeSort[T]) pmerge(arr *[]*T) {
	work := *arr
	// Base case: If the array has one or zero elements, it is already sorted.
	if len(work) <= 1 {
		return
...
func (m *MergeSort[T]) pjoin(left, right []*T) []*T {
	// Initialize the result slice with a capacity equal to the total length of the input slices.
	result := make([]*T, 0, len(left)+len(right))
