...
type Range interface {
	...
	RidePrev(n int) bool
	// DistanceFromStart returns the distance from the start
	DistanceFromStart() int
	// DistanceToEnd returns the distance from the end
	DistanceToEnd() int
}
...
func distanceFromStartAsc(r Range) int {
	return r.Current() - r.First()
}

func distanceFromStartDesc(r Range) int {
	return r.Current() - r.First()*-1
}

func distanceToEndAsc(r Range) int {
	return r.Last() - r.Current()
}

func distanceToEndDesc(r Range) int {
	return r.Last() - r.Current()*-1
}
