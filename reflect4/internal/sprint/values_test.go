...
func compareMessages(tt gtests.Loggable, result, expected string) bool {
	tt.Helper()
	cr := regexp.MustCompile(`<cyclic-ref\|0x[0-9a-f]+>`)
	ar := regexp.MustCompile(`<0x[0-9a-f]+>`)
	result = cr.ReplaceAllString(result, "<cyclic-ref|"+fmx.SGreen("0xt3st4ddr")+">")
	expected = cr.ReplaceAllString(expected, "<cyclic-ref|"+fmx.SGreen("0xt3st4ddr")+">")
	result = ar.ReplaceAllString(result, "<"+fmx.SGreen("0xt3st4ddr")+">")
	expected = ar.ReplaceAllString(expected, "<"+fmx.SGreen("0xt3st4ddr")+">")
