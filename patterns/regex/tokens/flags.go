...
var (
	...
	Global = Flag{Logical: symbols.OpenOf(grammar.ByteSymbol([]byte{'.'}))}
	// IgnoreCase i: Performs case-insensitive matching.
	IgnoreCase = Flag{Logical: symbols.OpenOf(grammar.ByteSymbol([]byte{'i'}))}
	// Multiline m: Enables ^ and $ to match the beginning and end of each line, respectively.
	Multiline = Flag{Logical: symbols.OpenOf(grammar.ByteSymbol([]byte{'m'}))}
